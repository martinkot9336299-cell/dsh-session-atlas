...
import { buildMergedGraph, compileContext, renderContextBlock, parseTurnNodeId, turnNodeId, wouldCreateCycle, stalenessDecisions, filterGraphByWorkspace } from './graph/core.mjs'
...
export function apply(ctx, config) {
  ...
  // ─── v0.2 Context Graph layer（蓝图 Phase 1，默认开启，可经 graphLayer:false 关闭）───
  const graphLayer = config?.graphLayer !== false
  const graphStore = graphLayer
    ? new GraphEventStore({
      eventsFile: config?.graphEventsFile ?? `${dirname(config?.dataFile ?? 'session-atlas.json')}/graph-events.jsonl`,
      snapshotFile: config?.graphSnapshotFile ?? `${dirname(config?.dataFile ?? 'session-atlas.json')}/graph-snapshot.json`,
    })
    : null
  const graphPending = new Map() // sessionId → { text, block, armedAt, fingerprint?, sourceNodeIds? } — pre-step 注入队列（一次性）
  const graphHooks = { recomputeStaleness: async () => {} } // graphLayer 块填充，API 段调用
  const factsCache = new Map() // sessionId → { lastSeq, count, facts } — merged graph 的折叠缓存
  const factsOf = (sessionId, events) => {
    const lastSeq = events.length > 0 ? events[events.length - 1].seq : -1
    const cached = factsCache.get(sessionId)
    if (cached !== undefined && cached.lastSeq === lastSeq && cached.count === events.length) return cached.facts
    const facts = foldSessionFacts(events)
    factsCache.set(sessionId, { lastSeq, count: events.length, facts })
    return facts
  }
  const mergedGraphSnapshot = () => buildMergedGraph(graphStore.read(), ctx.sessions.list(), factsOf)
  // 派生 turn 节点第一次被 Synapse 操作（归档/改名）时，先在事件存储里落一个存根
  const ensurePersistableNode = async id => {
    if (graphStore === null || graphStore.read().nodes[id] !== undefined) return
    const parsed = parseTurnNodeId(id)
    const stub = parsed !== null
      ? { id, type: 'turn', sessionId: parsed.sessionId, turn: parsed.turn, status: 'normal' }
      : { id, type: 'note', title: id, status: 'normal' }
    await graphStore.append('NODE_CREATED', { node: stub })
  }
  if (graphLayer) {
    // P1-2：会话事实单元注册进官方投影引擎（惰性折叠 + 检查点/恢复白得）
    const projections = ctx.get('sessionProjections')
    if (projections !== undefined) {
      ctx.effect(() => projections.register(sessionFactsDefinition()), 'session-atlas: sessionFacts projection')
    }
    // Phase 3：指纹冻结 → turn/end 落档 → 变更后重算过期
    // P1-1：fingerprint 冻结的是 arm 时「锚点（fromNodeId）上下文」的编译指纹，
    // anchorNodeId 一并落档；过期检测按锚点比对，消费轮自身对话增长不再参与。
    const awaitingGeneration = new Map() // sessionId → { fingerprint, anchorNodeId, sourceNodeIds, at }
    const recomputeStaleness = async () => {
      if (graphStore === null) return
      const merged = mergedGraphSnapshot()
      for (const decision of stalenessDecisions(graphStore.read(), merged)) {
        await graphStore.append('NODE_PATCHED', { id: decision.id, patch: decision.patch })
      }
    }
    ctx.on('session/event', (session, event) => {
      if (event?.type !== 'turn/end') return
      const awaiting = awaitingGeneration.get(session.id)
      if (awaiting === undefined) return
      awaitingGeneration.delete(session.id)
      const nodeId = turnNodeId(session.id, Number(event.data?.turn) || 0)
      void (async () => {
        try {
          await ensurePersistableNode(nodeId)
          await graphStore.append('NODE_PATCHED', {
            id: nodeId,
            patch: { generation: { fingerprint: awaiting.fingerprint, anchorNodeId: awaiting.anchorNodeId, sourceNodeIds: awaiting.sourceNodeIds, at: awaiting.at } },
          })
    ...
    // P1-6：pre-step 注入适配器 —— 仅当某会话被显式 arm 过才动手，其余原样透传
    ctx.on('agent/pre-step', async (payload, next) => {
      const agentId = payload?.agent?.id
      const pending = graphPending.get(agentId)
      if (pending === undefined) return next()
      graphPending.delete(agentId)
      // Phase 3：带指纹的注入同时冻结「生成时上下文」，等本轮 turn/end 落档
      if (pending.fingerprint != null) {
        awaitingGeneration.set(agentId, { fingerprint: pending.fingerprint, anchorNodeId: pending.anchorNodeId ?? null, sourceNodeIds: pending.sourceNodeIds ?? [], at: new Date().toISOString() })
      }
      const decision = await next()
      if (decision?.kind !== 'enter' || !Array.isArray(decision.messages)) return decision
      const text = pending.block !== null ? `${pending.block}\n\n${pending.text}` : pending.text
      decision.messages = [...decision.messages, {
        id: randomUUID(),
        role: 'user',
        source: { kind: 'session-atlas-context' },
        content: [{ type: 'text', text }],
      }]
      ctx.logger.info(`session-atlas: 已注入上下文块 → ${agentId} turn ${payload.turn} step ${payload.step}（${text.length} 字符）`)
      return decision
    })
    // 暴露给 API 段：变更后重算
    graphHooks.recomputeStaleness = recomputeStaleness
  }
  ...
  const api = async (req, res) => {
    try {
      ...
      // ─── v0.2 Context Graph API ───
      if (graphLayer && path === '/session-atlas/api/graph' && req.method === 'GET') {
        // Phase 4 D2：?workspaceId=<官方id> → 项目视图过滤（纯视图层，不改图语义：
        // 编译/注入/过期始终走全量图，本参数只影响这份读取）。
        const requestUrl = new URL(req.url ?? '/', 'http://dsh.local')
        ...
        if (workspaceId !== null && workspaceId !== '') {
          ...
          if (workspace === undefined) throw new NotFoundError(`工作区 ${workspaceId} 不存在`)
          return sendJson(res, 200, { graph: filterGraphByWorkspace(mergedGraphSnapshot(), workspace.sessionIds), workspace: { id: workspace.id, title: workspace.title, path: workspace.path } })
        }
        return sendJson(res, 200, { graph: mergedGraphSnapshot() })
      }
      if (graphLayer && path === '/session-atlas/api/graph/nodes' && req.method === 'POST') {
        const body = await readJson(req)
        if (!['material', 'note', 'summary', 'artifact'].includes(body?.type)) throw new InputError('type 必须是 material / note / summary / artifact')
        const title = requiredText(body?.title, 200, 'title')
        const content = typeof body?.content === 'string' ? body.content.slice(0, 20_000) : ''
        const id = `n:${randomUUID()}`
        await graphStore.append('NODE_CREATED', { node: { id, type: body.type, title, content, status: 'normal' } })
        return sendJson(res, 201, { node: graphStore.read().nodes[id] })
      }
      const graphArchive = /^\/session-atlas\/api\/graph\/nodes\/([^/]+)\/archive$/i.exec(path)
      if (graphLayer && graphArchive !== null && req.method === 'POST') {
        const id = decode(graphArchive[1])
        const merged = mergedGraphSnapshot()
        if (merged.nodes[id] === undefined) throw new NotFoundError(`节点 ${id} 不存在`)
        await ensurePersistableNode(id)
        await graphStore.append('NODE_ARCHIVED', { id })
        await graphHooks.recomputeStaleness()
        return sendJson(res, 200, { node: graphStore.read().nodes[id] })
      }
      if (graphLayer && path === '/session-atlas/api/graph/edges' && req.method === 'POST') {
        const body = await readJson(req)
        const from = requiredText(body?.from, 300, 'from')
        const to = requiredText(body?.to, 300, 'to')
        const merged = mergedGraphSnapshot()
        if (merged.nodes[from] === undefined) throw new NotFoundError(`节点 ${from} 不存在`)
        if (merged.nodes[to] === undefined) throw new NotFoundError(`节点 ${to} 不存在`)
        if (wouldCreateCycle(merged, from, to)) throw new InputError('该连线会形成环，已拒绝（DAG 约束）')
        // 幂等：同 from/to/mode 的引用边已存在（重复点击同一轮）直接返回原边
        const duplicate = Object.values(merged.edges).find(edge => edge.from === from && edge.to === to && edge.mode === 'reference')
        if (duplicate !== undefined) return sendJson(res, 200, { edge: duplicate, deduped: true })
        const id = `r:${randomUUID()}`
        await graphStore.append('EDGE_CREATED', { edge: { id, from, to, mode: 'reference', createdAt: new Date().toISOString() } })
        await graphHooks.recomputeStaleness()
        return sendJson(res, 201, { edge: graphStore.read().edges[id] })
      }
      const graphEdge = /^\/session-atlas\/api\/graph\/edges\/([^/]+)$/i.exec(path)
      if (graphLayer && graphEdge !== null && req.method === 'DELETE') {
        const graphEdgeId = decode(graphEdge[1])
        if (graphStore.read().edges[graphEdgeId] === undefined) throw new NotFoundError(`边 ${graphEdgeId} 不存在`)
        await graphStore.append('EDGE_REMOVED', { id: graphEdgeId })
        await graphHooks.recomputeStaleness()
        return sendJson(res, 200, { ok: true })
      }
      const graphContext = /^\/session-atlas\/api\/graph\/context\/([^/]+)$/i.exec(path)
      if (graphLayer && graphContext !== null && req.method === 'GET') {
        const manifest = compileContext(mergedGraphSnapshot(), decode(graphContext[1]))
        return sendJson(res, 200, { manifest, preview: renderContextBlock(manifest) })
      }
      if (graphLayer && path === '/session-atlas/api/graph/arm-inject' && req.method === 'POST') {
        const body = await readJson(req)
        // fromNodeId 场景正文可空：注入块附在用户正常输入的消息之后，问题本体由用户
        // 在官方 composer / 画布内联 composer 里键入（Phase 2 交互模型）。
        const text = typeof body?.text === 'string' ? body.text.slice(0, 4_000) : (typeof body?.fromNodeId === 'string' && body.fromNodeId !== '' ? '' : requiredText(body?.text, 4_000, 'text'))
        let sessionId = typeof body?.sessionId === 'string' && body.sessionId !== '' ? body.sessionId : null
        if (sessionId === null && typeof body?.titleMatch === 'string' && body.titleMatch.trim() !== '') {
          const needle = body.titleMatch.trim()
          ...
          if (hit === undefined) {
            const known = sessions.slice(0, 10).map(session => {
              const snapshot = titleService?.get?.(session)
              return typeof snapshot?.title === 'string' ? snapshot.title : session.id
            })
            throw new NotFoundError(`没有标题含「${needle}」的会话。现有标题示例：${known.join(' / ')}`)
          }
          sessionId = hit.id
        }
        if (sessionId === null || ctx.sessions.get(sessionId) === undefined) throw new InputError('需要有效的 sessionId，或用 titleMatch 按标题定位')
        let manifest = null
        let block = null
        if (typeof body?.fromNodeId === 'string' && body.fromNodeId !== '') {
          manifest = compileContext(mergedGraphSnapshot(), body.fromNodeId)
          block = renderContextBlock(manifest)
        }
        graphPending.set(sessionId, {
          text, block, armedAt: new Date().toISOString(),
          fingerprint: manifest?.fingerprint ?? null,
          anchorNodeId: typeof body?.fromNodeId === 'string' && body.fromNodeId !== '' ? body.fromNodeId : null,
          sourceNodeIds: manifest?.sourceNodeIds ?? null,
        })
      ...
      const graphNodePatch = /^\/session-atlas\/api\/graph\/nodes\/([^/]+)$/i.exec(path)
      if (graphLayer && graphNodePatch !== null && req.method === 'PATCH') {
        const body = await readJson(req)
        const patch = {}
        if (body?.position !== null && typeof body?.position === 'object') {
          patch.position = { x: Math.round(Number(body.position.x) || 0), y: Math.round(Number(body.position.y) || 0) }
        }
        if (typeof body?.title === 'string' && body.title.trim() !== '') patch.title = body.title.trim().slice(0, 200)
        if (typeof body?.content === 'string') patch.content = body.content.slice(0, 20_000)
        if (Object.keys(patch).length === 0) throw new InputError('没有可更新的字段（position / title / content）')
        await ensurePersistableNode(decode(graphNodePatch[1]))
        await graphStore.append('NODE_PATCHED', { id: decode(graphNodePatch[1]), patch })
        if (patch.content !== undefined || patch.title !== undefined) await graphHooks.recomputeStaleness()
        return sendJson(res, 200, { node: graphStore.read().nodes[decode(graphNodePatch[1])] })
      }
      // Phase 3：保留旧结果——清除过期标记并把生成指纹重冻到当前上下文。
      // P1-1：有锚点的档案保持锚点不变、重冻锚点当前指纹（语义=「接受此刻的锚点
      // 上下文为基准，此后再变才过期」）；无锚点存量档案以节点自身为锚。
      const graphRefresh = /^\/session-atlas\/api\/graph\/nodes\/([^/]+)\/refresh-stale$/i.exec(path)
      if (graphLayer && graphRefresh !== null && req.method === 'POST') {
        const id = decode(graphRefresh[1])
        const merged = mergedGraphSnapshot()
        if (merged.nodes[id] === undefined) throw new NotFoundError(`节点 ${id} 不存在`)
        const existing = merged.nodes[id].generation
        const anchorId = typeof existing?.anchorNodeId === 'string' && existing.anchorNodeId !== '' && merged.nodes[existing.anchorNodeId] !== undefined
          ? existing.anchorNodeId
          : id
        const manifest = compileContext(merged, anchorId)
        await ensurePersistableNode(id)
        await graphStore.append('NODE_PATCHED', {
          id,
          patch: { status: 'normal', generation: { fingerprint: manifest.fingerprint, anchorNodeId: anchorId, sourceNodeIds: manifest.sourceNodeIds, at: new Date().toISOString() } },
        })
        return sendJson(res, 200, { node: graphStore.read().nodes[id] })
      }
